...
Garp is a **private studio tool** — Matt's kit for building and maintaining Safaii Studio client sites. It is not distributed as a product today: no public repo, no docs site, no community, no support surface — though Matt posts about it publicly as a showcase/build-in-public artifact, and a public product release is a possible *someday* (decided 2026-07-14), not a commitment. Until that decision is made deliberately, build nothing product-shaped. Its purpose is to make Matt better at the actual website work instead of fighting tooling.
...
- **`garp dev`** — runs build, serves `site/` over local HTTP, watches `content/ layouts/ components/ data/ static/ config.yaml` via fsnotify, rebuilds on change, and live-reloads the browser. Ships a **dev toolbar** (`devtools.go` + embedded `devtools/toolbar.js`, served at `/_garp/toolbar.js`): live reload via the SSE endpoint `/_garp/reload`, plus a **page-data inspector** — a shadow-DOM panel (the "g" button, bottom-right) showing the current page's resolved cascade with per-key provenance (frontmatter vs `_data.yaml` vs `data/*` vs `config.yaml`, from `/_garp/page?path=…`), its layout chain, and its collections. One script tag appended to HTML *responses* only — `site/` on disk is never touched, so the litmus test isn't grazed. Also serves the kitchen sink at `/_garp/sink`, regenerated per request (see Kitchen sink generation below). Prints the local URL. Takes `-port N` (default 8080); without an explicit `-port` it falls back to an OS-assigned port if 8080 is taken. The HTTP handler mirrors the host: a clean URL like `/about` falls back to `about.html`, and missing paths get the project's `404.html` with a 404 status, like Cloudflare Pages. (Renamed from `garp serve` 2026-07-13 — local development only, never a production server.)
...
- **Kitchen sink generation** (added 2026-07-14, from the Grease comparison — this is the Phase 3 "design-system reference page," pulled forward and upgraded from scaffolded to generated) — a reference page rendering the project's tokens (every custom property in `static/style.css`'s `@layer config` block, the parse contract) as swatches/bars/specimens, plus a fixed semantic-elements section styled by the site's own CSS. **Token values are never computed in Go** — previews are `var(--token)` references the browser resolves against the live stylesheet, so the page cannot drift. Two tiers sharing one generator (`sink.go`): `/_garp/sink` on the dev server (regenerated per request), and opt-in build synthesis — `sink: true` in `config.yaml` emits `kitchen-sink.html` (noindex) on every build, author-overridable like sitemap.xml (a `static/kitchen-sink.html` wins).
