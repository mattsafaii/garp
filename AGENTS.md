...
- **Kitchen sink generation** (added 2026-07-14, from the Grease comparison — this is the Phase 3 "design-system reference page," pulled forward and upgraded from scaffolded to generated) — a reference page rendering the project's tokens (every custom property in `static/style.css`'s `@layer tokens` block, the parse contract) as swatches/bars/specimens, plus a fixed semantic-elements section styled by the site's own CSS. **Token values are never computed in Go** — previews are `var(--token)` references the browser resolves against the live stylesheet, so the page cannot drift. Two tiers sharing one generator (`sink.go`): `/_garp/sink` on the dev server (regenerated per request), and opt-in build synthesis — `sink: true` in `config.yaml` emits `kitchen-sink.html` (noindex) on every build, author-overridable like sitemap.xml (a `static/kitchen-sink.html` wins).
