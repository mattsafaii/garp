...
- **`garp serve`** — runs build, serves `site/` over local HTTP, watches `content/ layouts/ components/ data/ static/ config.yaml` via fsnotify, rebuilds on change. Prints the local URL.
...
- **Collections are variables, not pages.** Gather content into `collections.<tag>` (and content-type groupings) exposed to templates. The generator NEVER auto-generates archive/taxonomy pages. A blog index is just `content/blog/index.md` with a layout that loops over `collections.blog`.
- **Templates:** Pongo2, using `extends`/`block`/`include`. Layouts live in `layouts/`, reusable fragments in `components/`.
