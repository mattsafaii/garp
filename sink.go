...
// @layer config block) rendered as swatches and specimens, plus a fixed
...
// token is one custom property declared in @layer config, with its raw
...
// extractConfigTokens returns the custom properties declared inside the
// stylesheet's @layer config block, in declaration order. The config
// layer is the contract (per the studio CSS conventions); a stylesheet
// without one yields no tokens.
func extractConfigTokens(css string) []token {
	body, ok := layerBlock(css, "config")
	if !ok {
		return nil
	}
	var tokens []token
...
// The layer *statement* form (`@layer config, reset, ...;`) has no body
...
func renderSinkPage(groups []tokenGroup, siteName string) string {
	...
	b.WriteString("<p>Every token from <code>static/style.css</code>'s <code>@layer config</code>, and the standard elements, rendered by this site's own stylesheet. Generated by garp — token values are live <code>var()</code> references, so this page is always current.</p>\n")

	if len(groups) == 0 {
		b.WriteString("<p><em>No tokens found — static/style.css has no <code>@layer config</code> block.</em></p>\n")
	}
...
func serveSink(root string, w http.ResponseWriter) {
	...
	fmt.Fprint(w, renderSinkPage(groupTokens(extractConfigTokens(string(css))), cfg.SiteName))
}
...
func synthesizeSink(root, outDir string, cfg *Config) (int, error) {
	...
	page := renderSinkPage(groupTokens(extractConfigTokens(string(css))), cfg.SiteName)
